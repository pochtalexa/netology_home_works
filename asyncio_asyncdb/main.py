...
from models import People, db, PG_DSN
...
async def save_resource(resource_name, resource_index):
    status_code, result = await make_request(f'{resource_name}/{resource_index}/')
    ...
    new_resource_data = await cook_data_for_db(result, resource_index)
    new_resource = await People.create(**new_resource_data)
...
async def main():
    ...
    await People.delete.gino.status()
